...
import nuevoviruz from "./theme/nuevoviruz.json" with { type: "json" }
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "nuevoviruz")
        draft.active = typeof active === "string" ? active : "nuevoviruz"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.nuevoviruz, store.mode)
    })
